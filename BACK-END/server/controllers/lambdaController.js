...
const lambdaController = {
    async shear(request, response, next) {
        if (!request.body.ARN) {
            const error = new Error('Error reading ARN!');
            error.status = 403;
        ...
        if (!memoryArray || !Array.isArray(memoryArray) || memoryArray.length === 0) {
            const error = new Error('Error with memory array!');
            error.status = 403;
        ...
        async function createNewVersionsFromMemoryArrayAndInvoke(inputArr, arn) {
            try {
                ...
                const error1 = new Error('Error creating new versions.');
                error1.status = 403;
        ...
        async function invokeSpecificVersion(version, payload) {
            try {
                ...
                const error1 = new Error('Error with invoking specific version.');
                error1.status = 512;
                error1.requestDetails = { body: request.body };
                return next(error);
            }
        ...
        async function getLogStreams(logGroupName) {
            ...
            try {
                ...
                const error1 = new Error('Error with fetching log streams.');
                error1.status = 512;
                error1.requestDetails = { body: request.body };
                return next(error);
            }
        ...
        async function getLogGroupsNew(funcName) {
            ...
            try {
                ...
                const error1 = new Error('Error with describing log streams.');
                error1.status = 512;
                error1.requestDetails = { body: request.body };
                return next(error);
            }
        ...
        async function seekReportsRecursively(events, soughtResults, params, 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        resultsArr = [], attempts = 0) {
            ...
            else if (resultsArr[1].length >= soughtResults)
                return resultsArr;
        ...
        if (logStreams && logGroupName) {
            for (const element of logStreams) {
                if (element.logStreamName) {
                    ...
                    const error1 = new Error('Error with reading logStream Name. This should not happen.');
                    error1.status = 512;
                    error1.requestDetails = { body: request.body };
                    return next(error1);
                }
            ...
            const error1 = new Error('Error with reading logStream Name or logGroup name. This should not happen.');
            error1.status = 512;
            error1.requestDetails = { body: request.body };
            return next(error1);
        }
...
function calculateCosts(resultObj) {
    const newObj = {};
